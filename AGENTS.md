...
- `docs/plan.md` — **the standalone source of truth**: design principles, note model, storage layout, screens, editor, canvas, friction system, AI integration, roadmap, risks.
  Read it before writing code.
- `docs/design/` — UI direction: the wireframe deck plus `wireframes-v0.md`, its full description — Part I is the operative spec (two screens: the table, v1; the logs, v0 — layout, palette, chrome, states), Part II the decision trail.
  Read Part I before writing any UI.
...
- When you (Codex) participate in a decision with the user, write the ADR as part of the same change; do not let decisions live only in conversation.
- When committing and a decision is unclear, ask the user why something was made the way it was so it can be documented in an ADR.
...
- **UI**: Dioxus 0.7 desktop.
  **Whenever Dioxus code is written or understood, first read `.claude/dioxus.md`** (the Dioxus 0.7 API reference shared with Claude Code): 0.7 changed every API — `cx`, `Scope`, and `use_state` are gone; use `use_signal`, `#[component]`, `rsx!`, `Routable`, `use_resource`.
...
- **Plain files are the source of truth.**
  The index is derived and must always be rebuildable from the `.typ` files; every note compiles standalone via the shared `template.typ`.
- **AI never writes prose in note files** (sole exception: the explicit `generated` type).
  Suggestions live only in the sidecar index; accepting a link suggestion = the user writes it (Tab-inserted ghost text), never an accept button that edits the file.
- **No hard blocks.**
  All friction (unsummarized captures, unresolved suggestions, dangling links, typeless notes) is visible debt in the open-loops panel, never a save-blocker.
...
- **v1 — the table**: canvas with persistent positions, semantic zoom, modal card editing, filters, auto-placement.
  The v1 list is the ceiling, not the floor.
...
- Codex is the primary implementation agent — code, tests, docs; the user directs, decides, and reviews (`adr/2026-08-codex-and-claude-run-in-parallel.md`).
...
- Code should be structured to avoid `expect` in production code as much as possible.
