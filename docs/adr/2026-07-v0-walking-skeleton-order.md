...
`roadmap-v0.md` orders v0 in 8 phases along dependencies, not feature bullets:
...
3. The split-view editor is built *first* as a stepping stone (phase 5), hybrid block editing upgrades it afterwards (phase 6) — split view is a milestone on the path, not only a fallback.
