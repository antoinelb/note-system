...
- **Unplaced notes land near the canvas origin** (a 4-wide grid as the spacing scheme — the grid itself is incidental), deterministic and visible where the viewport starts, dumb and honest until phase 8's auto-placement. **Within a session each unplaced note keeps the first slot it was given** (`table::Fallback`), so placing one card never shuffles the rest under the cursor; the compaction happens only on the next launch. The slots are session state, never written to the store — a missing entry still means "not yet placed", which phase 8 depends on.
