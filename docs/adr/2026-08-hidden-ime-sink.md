...
- The sink's value is never read; printable keys are prevented so it stays empty. (It used to remount with the block; since `adr/2026-09-the-sink-outlives-the-active-block.md` it lives beside the blocks and never remounts, because the remount lost the focus and dropped the keys typed before the fresh grab landed.)
