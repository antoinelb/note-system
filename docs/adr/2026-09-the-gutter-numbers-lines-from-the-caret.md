...
**A heading's number is centred on the heading's row.** The number's line-height was one prose line, so beside a heading's taller row it sat at the top. `.note-blocks` now names the row's size as `--row-size`, the prose size by default and each heading level's own size where the heading rule sets it, and the number's line-height is that size times the leading: the row's own height, and line-height's half-leading centres the digits in it.
