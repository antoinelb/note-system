...
`makefile` excludes `main.rs` from coverage, so putting Dioxus components there would exempt the UI from the 100% rule of `adr/2026-07-couverture-100-pourcent-lignes.md`.
...
Components live in `src/ui.rs` and are covered like any other module. `main.rs` keeps only `dioxus::launch`.
