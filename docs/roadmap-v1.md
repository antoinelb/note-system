...
Ordering rationale: `adr/2026-08-v1-walking-skeleton-order.md`.
...
## Phase 0 — Positions store (Rust, no UI)
...
- [ ] Semantics: id → (x, y); a missing entry means "not yet placed" — never an error, phase 7's auto-placement decides later.
...
## Phase 1 — Walking skeleton: the table at rest
...
Spec: wireframe state 6a. `Screen::Table` finally mounts (the "table mounts in v1" note in `ui.rs` ends here); the links-footer and Ctrl+Enter "wait for v1" branches end in phase 2, not here.
...
- [ ] Cards at titles zoom: ~176px wide (mockup 170–180, normalized to ×4), uppercase mono type label over a sans title, absolutely positioned from phase 0; unplaced notes stack somewhere deterministic and visible — dumb and honest until phase 7.
...
- [ ] Drag a card to move it; the position persists through phase 0's debounced write.
...
## Phase 3 — CRUD for permanent notes
...
- [ ] Create: a keystroke summons a type picker (the link-picker overlay pattern — the design has no buttons), instantiates the per-type template (v0 phase-5 machinery), and opens the new card's sheet.
  - [ ] The keystroke, the picker's shape, and where the new card lands before phase 7 auto-places it (suggestion: viewport centre). **→ ADR**
...
## Phase 4 — Constellations: link edges
...
## Phase 6 — Findability: filters and jump
...
## Phase 7 — Auto-placement and cluster arrange
...
- [ ] Canvas with persistent positions that survive index rebuilds (phases 0–1)
- [ ] Two-level semantic zoom: titles ⇄ rendered bodies (phase 5)
- [ ] Tethered writing sheet on card click, dimmed table behind it (phase 2)
- [ ] Tag & type filters, type colours, jump-to-note (phases 1, 6)
- [ ] Auto-placement of new notes near linked ones; on-demand cluster arrange (phase 7)
- [ ] Capture/generated visual treatment; `generated` type defined (phase 1)
- [ ] Permanent-note CRUD in the app; the vault grows without leaving it (phases 2–3)
