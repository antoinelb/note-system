...
- **The modal layer grew the keys daily writing earned**: `s`/`S`, `R` replace mode, vim-surround (`cs`/`ds`/`ys`/`yss`/visual `S`), a linewise `V` highlight, and `j`/`k` that walk the wrapped lines the webview draws through an injected geometry seam (`adr/2026-08-clusters-noun-belongs-to-s.md`, `adr/2026-08-replace-mode-session-and-backspace.md`, `adr/2026-08-surround-pair-set-and-padding.md`, `adr/2026-08-v-highlight-covers-whole-lines.md`, `adr/2026-08-visual-line-j-k-through-a-geometry-seam.md`).
...
- Command palette: Ctrl+P fuzzy-searches every app command by name, Obsidian-style (first step — `adr/2026-08-command-palette-first-in-v1.md`)
...
- Owned-caret block widget replacing the active block's textarea (composition, clipboard, selection hand-rolled; undo waits for phase 5, `adr/2026-08-no-stopgap-undo-in-phase-0.md`); the caret's shape becomes the mode indicator
