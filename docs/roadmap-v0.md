...
- The only goal is building the note system (`adr/2026-07-goal-build-only.md`) — Claude is the implementation agent and Antoine directs and decides (`adr/2026-08-implementation-is-claudes-job.md`).
...
- [x] In-app capture: hotkey + paste → new file in `capture/` from the capture template, no required fields. Ctrl+Shift+V reads through the injected native clipboard seam (`adr/2026-08-clipboard-reads-are-native.md`) and reports the new id as a notice; the capture itself is never opened, since the centre pane shows time notes only.
