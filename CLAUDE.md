...
- **Nine permanent types**, `course` the ninth (a lecture is a `source` linking its course), and **`due: "YYYY-MM-DD"` is a `#meta` field any note may carry**: the open-loops list names a note overdue or due within seven days, last in the list, one date spoken where the date is the debt — the one amendment to the loops' "no ages" vocabulary (`adr/2026-09-course-type-and-due-loops.md`).
...
- **A key that beats an overlay's focus grab belongs to the overlay.** Every overlay grabs focus in an async `onmounted`; a bare key that reaches the sink or a pane while one is up is relayed into its query (or dropped when it has none) and never reaches the grammar — Escape and the chords still pass. Enter is dropped, not forwarded: the known ceiling (`adr/2026-09-overlay-keys-relay-before-focus-lands.md`).
